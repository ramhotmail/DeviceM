...
function PrintCard({ device, origin }: { device: any; origin: string }) {
  ...
  const row = (label: string, value: any) => <div className="flex min-h-[0.48cm] flex-1 items-center justify-between border-b border-slate-300 px-[0.06cm] text-[8px] leading-tight"><span className="ml-1 whitespace-nowrap font-bold text-slate-700">{label}:</span><span className="max-w-[3.45cm] overflow-hidden text-ellipsis whitespace-nowrap font-bold text-slate-950">{value || '—'}</span></div>;
  return <article className="device-print-card flex h-[13cm] w-[6cm] flex-col overflow-hidden border-2 border-blue-950 bg-white p-[0.18cm] text-slate-950">
    ...
    <div className="flex flex-1 flex-col justify-between overflow-hidden">{row('اسم الجهاز', device.device_name)}{row('كود الجهاز', device.device_code)}{row('المكان', device.location)}{row('الماركة', device.brand)}{row('الموديل', device.model)}{row('الرقم المسلسل', device.serial_number)}{row('الوكيل', device.agent_company)}{row('شركة الصيانة', device.maintenance_company)}{row('حالة الصيانة', device.maintenance_status)}{row('بداية التعاقد', device.contract_start)}{row('نهاية التعاقد', device.contract_end)}{row('مسؤول الصيانة', device.maintenance_officer)}{row('هاتف الصيانة', device.maintenance_phone)}</div>
    <footer className="mt-[0.08cm] flex items-end justify-between"><div className="text-[7px] font-bold leading-4 text-blue-950">امسح الكود لعرض الجهاز<br />أو الإبلاغ عن عطل</div><div className="border border-slate-300 bg-white p-[1px]"><QRCodeSVG value={qrUrl} size={52} /></div></footer>
  </article>;
}
